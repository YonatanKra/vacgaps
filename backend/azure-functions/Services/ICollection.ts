...
export function patchMongoCollection<T extends BaseEntity>(
  mongoCollection: Collection<any>
): ICollection<T> {
  const patchedCollection = (mongoCollection as unknown) as ICollection<T> &
    Collection<any>;
  patchedCollection.findMany = function (
    query: FilterQuery<Partial<T>>,
