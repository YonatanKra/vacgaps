...
import { Entity } from "../Models/BaseEntity";
...
export class CollectionProvider {
  ...
  public static async get<T>(collectionName: string): Promise<ICollection<T>> {
    if (!CollectionProvider.collections[collectionName]) {
      CollectionProvider.collections[collectionName] = await CollectionProvider.createCollection<T>(collectionName);
    }

    return CollectionProvider.collections[collectionName];
  }

  private static async createCollection<T>(collectionName: string): Promise<ICollection<T>> {
    if (CollectionProvider.mongoDb == null) {
      CollectionProvider.mongoDb = CollectionProvider.createMongoDb();
    }
    const mongoCollection = patchMongoCollection<Entity<T>>((await CollectionProvider.mongoDb).collection(collectionName));
  ...
  private static async createMongoDb(): Promise<Db> {
    ...
    const mongoClient = await MongoClient.connect(CollectionProvider.settings.mongoConnectionString,
      { useUnifiedTopology: true, useNewUrlParser: true, tlsAllowInvalidCertificates: this.settings.allowSelfSignedMongoCert });
    
    return mongoClient.db(this.settings.getVacciDatabase);
  }
}
