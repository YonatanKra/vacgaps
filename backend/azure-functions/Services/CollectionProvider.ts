...
export class CollectionProvider {
  ...
  public static async get<T extends BaseEntity>(collectionName: string): Promise<ICollection<T>> {
  ...
  private static async createCollection<T extends BaseEntity>(collectionName: string): Promise<ICollection<T>> {
    if (CollectionProvider.mongoDb == null) {
      CollectionProvider.mongoDb = CollectionProvider.createMongoDb();
    }
    const mongoCollection = patchMongoCollection<T>((await CollectionProvider.mongoDb).collection(collectionName));
